...
const testDbPath = path.join(__dirname, 'data', 'test-accounts.db');
...
async function runTests() {
  try {
    ...
    const addedIds = [];
    testAccounts.forEach(async (account, index) => {
    ...
    await db.updateAccount(updateId, {
      service_name: 'Google (Updated)',
      username: 'updated@gmail.com',
      secret_key: updatedSecret
    });
    const updatedAccount = await db.getAccountById(updateId);
